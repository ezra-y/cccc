...
pub(super) fn resolve_operation(request: &DaemonRequest) -> Option<Operation> {
    let handler = match request.op.as_str() {
        "web_model_chat_create" => {
            |home: &HomeLayout, request: &DaemonRequest| onboard(home, request, true)
        }
        "web_model_chat_bind" => {
            |home: &HomeLayout, request: &DaemonRequest| onboard(home, request, false)
        }
        _ => return None,
    };
    Some(Operation::new(GlobalWrite, handler))
}
...
fn op(home: &HomeLayout, name: &str, args: Value) -> OpResult {
    ...
    super::group_creation::resolve_operation(&request)
        .or_else(|| super::groups::resolve_operation(&request))
        .or_else(|| super::actors::resolve_operation(&request))
